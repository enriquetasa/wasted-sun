...
def day_row_from_cube_rows(
    ...
    *,
    ...
    if not rows:
        return None
    qh_mwh, qh_eur = merge_cube_rows(rows, n_slots)
    total_mwh = sum(qh_mwh[:n_slots], start=Decimal("0"))
    total_eur = sum(qh_eur[:n_slots], start=Decimal("0"))
    if total_mwh == 0 and not any(qh_mwh[:n_slots]):
        return None
    out: dict = {"date_day": day, "total_mwh": total_mwh, "total_eur": total_eur}
    for key, val in zip(qh_column_names(n_slots), qh_mwh[:n_slots], strict=True):
        out[key] = val
    for key, val in zip(qh_eur_column_names(n_slots), qh_eur[:n_slots], strict=True):
        out[key] = val
    return out
